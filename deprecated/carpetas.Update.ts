...
const Collection = async () => {
  ...
  const demandados = db.collection("Demandados");
  return demandados;
};

export async function EditDemandado({ demandado }: { demandado: IntCarpetas }) {
  try {
...
export async function fetchProcesoRama({ carpeta }: { carpeta: IntCarpetas }) {
  try {
    ...
    const ids = procesos.map((proceso) => proceso.idProceso);

    const nIds = ids.map((i) => {
      const newCarpeta: IntCarpetas = {
        ...carpeta,
        idProceso: i,
      };
  ...
  setTimeout(
    async () =>
      await fetchProcesoRama({
        ...
        async (carpetaArreglada) => {
          console.log(carpetaArreglada);
          rows.push(carpetaArreglada);
          fs.writeFile("./newCarpetas.json", JSON.stringify(rows));
