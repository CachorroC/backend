...
llaves.forEach((llave, index, array) => {
  setTimeout(() => {
    ...
    if (index + 1 === array.length - 1) {
      const Procesos = rawprocesos.flatMap((proceso) => proceso);
      lastIteration(reqNoOk /*? reqNoOk*/, Procesos, missingProcesos);
    }
  }, index * 1000);
});

function fetchProcesoSuccess(output: intProceso[]) {
  console.log(output);
  fps.push(output);
  fs.writeFileSync("successful.fetchConsultas.json", JSON.stringify(fps));
  return;
}
function fetchProcesoError(error: unknown, llave: string) {
  console.log(error);
  fpe.push(error);
...
function lastIteration(a: string[], b: intProceso[], c: string[]) {
  fs.writeFileSync(
