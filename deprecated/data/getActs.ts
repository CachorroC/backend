...
fs.mkdir("src/json/out/actuaciones", {
  recursive: true,
});

const actuaciones: intConsultaActuaciones[] = [];
...
const fetchActuaciones = async (idProceso: number, iOfA: string) => {
  ...
  console.log(req.status.toString());
  if (!req.ok) {
    const res = {
      status: req.status,
  ...
  if (req.ok) {
    ...
    siHay.push(res);
    console.log(res);
    fs.writeFile("actuacionesSiHay.json", JSON.stringify(siHay));
  ...
  const res = (await req.json()) as intConsultaActuaciones;
  actuaciones.push(res);
  fs.writeFile("actuaciones.json", JSON.stringify(actuaciones));
...
const forEachProceso = procesos.forEach((proceso, index, procArr) => {
  const helpers: (intHelper | { status: number; message: string })[] = [];
