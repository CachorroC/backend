...
export async function fetchActuacionesByidProceso(
  idProceso: number
) {
  ...
  if (!request.ok) {
    throw new Error (
      'fetch failed @ fetchActuacionesByidProceso'
    );
  }
...
export async function carpetaBuild(
  ...
  if (carpeta.idProceso) {
    ...
    for (const x of idProceso) {
      const request = await fetchActuacionesByidProceso (
        x
      );
      actuaciones.set (
        x,
...
const first_promise = new Promise (
  (
    resolve, reject
  ) => {
    setTimeout (
...
const second_promise = new Promise (
  (
    resolve, reject
  ) => {
    setTimeout (
...
const third_promise = new Promise (
  (
    resolve, reject
  ) => {
    setTimeout (
