...
export function arrayMergerByllaveProceso({
  ...
  b.forEach((carpeta, i, arr) => {
    const { _id, idProceso, llaveProceso } = carpeta;

    const isZero = idProceso === 0;

    const keyName = isZero ? llaveProceso : idProceso;
    return carpetasMap.set(keyName, {
      ...carpeta,
      Cname: keyName,
  ...
  a.forEach((proceso, i, arr) => {
    const { idProceso, llaveProceso } = proceso;

    const getByidProceso = carpetasMap.get(idProceso);

    const getByllaveProceso = carpetasMap.get(llaveProceso);

    const keyName = getByidProceso ? idProceso.toString() : llaveProceso;

    const keyValue = getByidProceso ?? getByllaveProceso;
    return procesosMap.set(keyName, {
      ...keyValue,
      Pname: keyName,
  ...
  const mergedArr = Array.from(procesosMap.values());
...
fs.writeFile("newMergedArrayPrcCarp.json", JSON.stringify(mrg));
