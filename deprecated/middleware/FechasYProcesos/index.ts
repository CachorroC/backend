import { intFecha, monCarpetaDemandado } from "#@/interface/demandados";
import { IntActuaciones, intConsultaActuaciones } from "#@/interface/procesos";
...
export async function fetchFechasMap(procesos: monCarpetaDemandado[]) {
  ...
  for (let p = 0; p < procesos.length; p++) {
    ...
    if (actuaciones.acts) {
      const fecha: intFecha = {
        ...proceso,
        fecha: actuaciones.acts[0].fechaActuacion,
      };

      fechas.set(fecha._id, fecha);
      console.log(fechas);
    }
    fechas.set(proceso._id, proceso);
    console.log(fechas);
  ...
  console.log(fechas.size);
  console.log(procesosMap.size);
  console.log(procesosMap);
}

export const Procesos: monCarpetaDemandado[] = [
  {
...
const req = fetchFechasMap(Procesos)
  .then((res) => {
    console.log(res);
  })
  .catch((rej) => {
