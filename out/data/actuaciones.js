...
exports.TipoConsulta = exports.Departamento = void 0;
...
})(Departamento = exports.Departamento || (exports.Departamento = {}));
var TipoConsulta;
(function (TipoConsulta) {
    TipoConsulta["NumeroRadicacion"] = "NumeroRadicacion";
})(TipoConsulta = exports.TipoConsulta || (exports.TipoConsulta = {}));
const procesos = [
    {
        cantFilas: -1,
        departamento: "BOGOTÁ",
    ...
    },
    {
        ...
        sujetosProcesales: "Demandante: BANCOLOMBIA S.A. | Demandado: HECTOR ALONSO TORRES RIOS ",
    },
    {
        cantFilas: -1,
        departamento: "BOGOTÁ",
    ...
    },
    {
        ...
        sujetosProcesales: "Demandante: BANCOLOMBIA S.A. | Demandado: JULIO RENE SANCHEZ ARGOTE ",
    },
    {
        cantFilas: -1,
        departamento: "BOGOTÁ",
...
const fullfiledActuacionesMap = [];
const fullfiledActuacionesWRMap = [];
const noFullfiledActuacionesWRMap = [];
const notFullfiledActuacionesMap = [];
const fullfiledActuaciones = [];
...
const rawReq = [];
const rawReqWR = [];
const fetchActuaciones = (idProceso, iOfA) => __awaiter(void 0, void 0, void 0, function* () {
    fs.mkdir(`src/json/out/actuaciones/${idProceso}`, { recursive: true });
    const req = yield fetch(`https://consultaprocesos.ramajudicial.gov.co:448/api/v2/Proceso/Actuaciones/${idProceso}`);
    ...
    if (!req.ok) {
        ...
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/reqNoOk.actuaciones.json`, JSON.stringify(reqNoOk));
    }
    if (req.ok) {
        const res = yield req.json();
        const actuaciones = res.actuaciones;
        if (actuaciones.length === 0) {
            const hasNoActuaciones = Object.assign(Object.assign({}, res), { idProceso: idProceso, iOfA: iOfA });
            reqOknoActuaciones.push(hasNoActuaciones);
            fs.writeFile(`src/json/out/actuaciones/${idProceso}/reqOknoActuaciones.actuaciones.json`, JSON.stringify(reqOknoActuaciones));
        }
        if (actuaciones.length > 0) {
            const hasActuaciones = Object.assign(Object.assign({}, res), { idProceso: idProceso, iOfA: iOfA });
            reqOkHasActuaciones.push(hasActuaciones);
            fs.writeFile(`src/json/out/actuaciones/${idProceso}/reqOkhasActuaciones.actuaciones.json`, JSON.stringify(reqOkHasActuaciones));
        }
        const requestOk = Object.assign(Object.assign({}, res), { idProceso: idProceso, iOfA: iOfA });
        reqOk.push(requestOk);
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/reqOk.actuaciones.json`, JSON.stringify(reqOk));
    }
    if (req.body) {
        const request = Object.assign(Object.assign({}, req.body), { idProceso: idProceso, iOfA: iOfA });
        reqOk.push(request);
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/reqOk.body.actuaciones.json`, JSON.stringify(reqOk));
    }
    const request = {
        idProceso: idProceso,
        iOfA: iOfA,
    ...
    rawReq.push(request);
    fs.writeFile(`src/json/out/actuaciones/${idProceso}/rawRequest.actuaciones.json`, JSON.stringify(rawReq));
});
const fetchActuacioneswithreturn = (idProceso, iOfA) => __awaiter(void 0, void 0, void 0, function* () {
    fs.mkdir(`src/json/out/actuaciones/${idProceso}`, { recursive: true });
    const req = yield fetch(`https://consultaprocesos.ramajudicial.gov.co:448/api/v2/Proceso/Actuaciones/${idProceso}`);
    console.log(req.status);
    if (!req.ok) {
        ...
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.reqNoOk.actuaciones.json`, JSON.stringify(reqNoOkWR));
        return errorMaker;
    }
    if (req.ok) {
        const res = yield req.json();
        ...
        if (actuaciones.length === 0) {
            ...
            fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.reqOknoActuaciones.actuaciones.json`, JSON.stringify(reqOknoActuacionesWR));
        ...
        if (actuaciones.length > 0) {
            ...
            fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.reqOkhasActuaciones.actuaciones.json`, JSON.stringify(reqOkHasActuacionesWR));
            return hasActuaciones;
        }
        const requestOk = Object.assign(Object.assign({}, res), { idProceso: idProceso, iOfA: iOfA });
        reqOkWR.push(requestOk);
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.reqOk.actuaciones.json`, JSON.stringify(reqOkWR));
        return requestOk;
    }
    if (req.body) {
        const request = Object.assign(Object.assign({}, req.body), { idProceso: idProceso, iOfA: iOfA });
        reqOkWR.push(request);
        fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.reqOk.body.actuaciones.json`, JSON.stringify(reqOkWR));
        return request;
    }
    const request = {
        idProceso: idProceso,
    ...
    fs.writeFile(`src/json/out/actuaciones/${idProceso}/WR.rawRequest.actuaciones.json`, JSON.stringify(rawReqWR));
    return request;
});
const forEachProceso = procesos.forEach((proceso, index, procArr) => {
    const iOfA = `${index}-${procArr.length}`;
    setTimeout(() => {
        const idProceso = proceso.idProceso;
        fetchActuaciones(idProceso, iOfA).then((fullfiled) => {
            const onFullfiled = {
                fullfiled: fullfiled,
                idProceso: idProceso,
                iOfA: iOfA,
            ...
            fs.writeFile("src/json/out/actuaciones/fullfiled.actuaciones.json", JSON.stringify(fullfiledActuaciones));
        }, (err) => {
            console.log(JSON.stringify(err));
            const notFullfiled = Object.assign(Object.assign({}, err), { idProceso: idProceso, iOfA: iOfA });
            notFullfiledActuaciones.push(notFullfiled);
            fs.writeFile("src/json/out/actuaciones/notFullfiled.actuaciones.json", JSON.stringify(notFullfiledActuaciones));
        });
        fetchActuacioneswithreturn(idProceso, iOfA).then((ff) => {
            const onFullfiledwr = Object.assign(Object.assign({}, ff), { idProceso: idProceso, iOfA: iOfA });
            fullfiledActuacionesWR.push(onFullfiledwr);
            fs.writeFile("src/json/out/actuaciones/WRfullfiled.actuaciones.json", JSON.stringify(fullfiledActuacionesWR));
            return onFullfiledwr;
        }, (err) => {
            console.log(JSON.stringify(err));
            const notFullfiledwr = Object.assign(Object.assign({}, err), { idProceso: idProceso, iOfA: iOfA });
            noFullfiledActuacionesWR.push(notFullfiledwr);
            fs.writeFile("src/json/out/actuaciones/WRnotFullfiled.actuaciones.json", JSON.stringify(noFullfiledActuacionesWR));
...
forEachProceso;
const messyMap = procesos.map((proceso, index, array) => {
    ...
    fetchActuaciones(idProceso, iOfA).then((fullfiled) => {
        ...
        fs.writeFile("src/json/out/actuaciones/fullfiled.Map.actuaciones.json", JSON.stringify(fullfiledActuacionesMap));
    }, (err) => {
        console.log(JSON.stringify(err));
        const notFullfiled = Object.assign(Object.assign({}, err), { idProceso: idProceso, iOfA: iOfA });
        notFullfiledActuacionesMap.push(notFullfiled);
        fs.writeFile("src/json/out/actuaciones/notFullfiled.Map.actuaciones.json", JSON.stringify(notFullfiledActuacionesMap));
    });
    return fetchActuacioneswithreturn(idProceso, iOfA).then((ff) => {
        const onFullfiledwr = Object.assign(Object.assign({}, ff), { idProceso: idProceso, iOfA: iOfA });
        fullfiledActuacionesWRMap.push(onFullfiledwr);
        fs.writeFile("src/json/out/actuaciones/WRfullfiled.Map.actuaciones.json", JSON.stringify(fullfiledActuacionesWRMap));
        return onFullfiledwr;
    }, (err) => {
        console.log(JSON.stringify(err));
        const notFullfiledwr = Object.assign(Object.assign({}, err), { idProceso: idProceso, iOfA: iOfA });
        noFullfiledActuacionesWRMap.push(notFullfiledwr);
        fs.writeFile("src/json/out/actuaciones/notFullfiledWR.Map.actuaciones.json", JSON.stringify(noFullfiledActuacionesWRMap));
        return notFullfiledwr;
    });
});
messyMap;
