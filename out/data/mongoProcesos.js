...
const polishProcesos_js_1 = require(
    '#@/middleware/polishProcesos.js'
);
const rawprocesos_fetchConsultas_json_1 = __importDefault(
    require(
        '../json/rawprocesos.fetchConsultas.json'
    )
);
...
function run() {
    return __awaiter(
        ...
        void 0,
        function* () {
            const updateProcesosArray =
            rawprocesos_fetchConsultas_json_1.default.map(
                (
                    proceso
                ) => {
                    const newProcesoBuilder = {
                        llaveProceso: proceso.llaveProceso,
                        sujetosProcesales: (0, polishProcesos_js_1.fixDemandado)(
            ...
            try {
                ...
                const result = yield foods.insertMany(
                    updateProcesosArray,
                    {
                        ordered: true,
                    }
