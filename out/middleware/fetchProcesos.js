...
exports.getProcesosRama = llaves.forEach((llave, index) => {
    setTimeout(() => fetchProcesoRama(llave).then((intConsultaNumeroRadicacion) => console.log(JSON.stringify(intConsultaNumeroRadicacion)), (error) => fs.writeFile("src/middleware/error.fetchProcesos.json", JSON.stringify(error))), index * 1000);
});
