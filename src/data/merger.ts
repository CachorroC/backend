...
const Carpetas: IntCarpetaDemandado[] = [
  {
    Carpeta: 14,
    Demandado: {
      Id: 1077969975,
...
/*
export function mergerByllaveProceso (
  {
    carpetas,
    actuaciones,
  }: {
    carpetas: IntCarpetaDemandado[];
  ...
  carpetas.forEach(
    (
      ...
      const selfish = map.get(
        carpeta.llaveProceso
      );
      console.log(
        selfish
      );
      const settingspray = map.set(
        carpeta.llaveProceso,
        {
          ...carpeta,
          ...selfish,
...
const acts = allActuaciones.map(
  (
    consAct
  ) => {
    if ( consAct.actuaciones ) {
      return consAct.actuaciones;
    }
    return [];
  }
).flat();
...
const unified = mergerByllaveProceso(
  {
    carpetas: Carpetas,
    actuaciones: acts,
  }
...
export function newFetchFechas (
  {
    carpetas
  }: {
    carpetas: IntCarpetaDemandado[]
                ...
                getActuacionesByidProceso(
                  ...
                  (
                    ...
                    if ( res.acts ) {
                      const fecha = {
                        ...carpeta,
                        fecha: res.acts[ 0 ].fechaActuacion
                      };
                      fechas.push(
                        fecha
                      );
