...
const missingProcesos: unknown[] = [];
const nonMissingProcesos: unknown[] = [];
...
export const polishProcesos = getProcesosfromRadicados.map(
  ( procesos, index, array ) => {
    if ( array.length === 0 ) {
      ...
      fs.writeFile(
        "src/middleware/missingProcesos.fixProcesos.json",
        JSON.stringify( missingProcesos )
      );
    ...
    const eachProceso = procesos.forEach( ( proceso ) => {
      nonMissingProcesos.push( proceso );
       fs.writeFile(
      "src/middleware/cleanProcesos.fixProcesos.json",
      JSON.stringify( nonMissingProcesos )
    );
    } );
